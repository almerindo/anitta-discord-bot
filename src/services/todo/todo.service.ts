...
export class TodoService {
  async addTodo(userId: string, username: string, code: string, description: string): Promise<ITodo> {
    ...
    async updateTodoStatus(userId: string, code: string, status: ETodoStatus): Promise<ITodo | null> {
        ...
        return await TodoModel.findOneAndUpdate({ userId: `<@${userId}>`, code }, update, { new: true });
    }
  ...
  async updateTodoText(userId: string, code: string, newDescription: string): Promise<ITodo | null> {
